...
export default function Recipe({
  ...
  return (
    <div
      key={index}
      className="relative pb-8 bg-gray-200 rounded-xl border-2 border-stone-700 p-3 hover:cursor-pointer"
    ...
    >
      ...
      <h2 className="font-bold text-xl text-center">{recipe.title}</h2>
      {recipe.image ? <img src={recipe.image} className="rounded-xl w-52 h-52 mx-auto my-3" /> : null}
      <h2 className="mt-3 font-bold">- 재료</h2>
      <p className="whitespace-pre-line line-clamp-2">{recipe.ingredients}</p>
      <h2 className="mt-3 font-bold">- 과정</h2>
      <p className="whitespace-pre-line line-clamp-2">{recipe.method}</p>
      <div
